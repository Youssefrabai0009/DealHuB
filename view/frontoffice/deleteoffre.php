...
<body>
  ...
  <div class="container mt-4">
    ...
    <!-- Tableau Offres (masqué au départ) -->
    <div id="offresTable" style="display: none;" class="mt-4">
      <h3 class="text-center mb-4">Vos Offres</h3>
      <div class="text-center mb-3">
  <button class="btn btn-success" onclick="trierTableauParDate()">Trier par Date</button>
</div>
  ...
  const table = document.querySelector("#offresTable table tbody");
  const rows = Array.from(table.querySelectorAll("tr"));
  
  rows.sort((a, b) => {
    const dateA = new Date(a.cells[1].innerText.trim());
    const dateB = new Date(b.cells[1].innerText.trim());
    return dateA - dateB; // Tri croissant
  });

  // Vide le tbody et remet les lignes triées
